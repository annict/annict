...
1. `docs/plans/template.md` をコピーして `docs/plans/1_doing/sync-YYYYMMDD.md` を作成する（YYYYMMDD は実行日）
2. タイトルを「プロジェクト間ファイル同期 YYYY/MM/DD」とする
3. 不要なセクション（「要件」「実装ガイドラインの参照」「非機能要件」「採用しなかった方針」「参考資料」）を削除する
...
差分レポートの採用方針チェックボックスに、推奨する選択肢に `(Recommended)` を付記する。
