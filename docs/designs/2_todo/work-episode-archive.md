...
- [ ] **2-1**: [Go] 作品アーカイブ機能の実装
  ...
  - 作品アーカイブユースケースの実装
  - Annict DB のハンドラー実装
  - 管理画面 UI の実装（アーカイブボタン、archive_message 入力欄）
  - **想定ファイル数**: 約 6 ファイル（実装 4 + テスト 2）
  - **想定行数**: 約 250 行（実装 150 行 + テスト 100 行）

- [ ] **2-2**: [Go] エピソードアーカイブ機能の実装
...
### フェーズ 4: エピソードなし作品へのエピソード追加

- [ ] **4-1**: [Go] エピソードなし作品にエピソードを作成するバッチ処理
