...
- [x] **2-3**: EpisodeRecord・WorkRecordモデルからfacebook関連コードを削除
...
- [x] **4-1**: Facebook共有ボタンコンポーネントとJavaScriptを削除
...
- [x] **5-1**: Deprecated Facebookサービス・クエリを削除
  ...
  - `Provider#token_available`スコープを削除（`User#authorized_to?`削除に伴い呼び出し元がなくなるため不要）
  - `Settings::ProvidersController#destroy`アクションを削除（プロバイダー削除機能は不要）
  - `Settings::UsersController`から`providers.delete_all`を削除
  - 関連するルーティング、I18nを更新
  - （`sns_image_service.rb`は`og:image`取得Rakeタスクで使用中のため残す）
  - （`Provider`モデル、ファクトリー、`has_many :providers`は他のテストで使用されるため残す）
  - **想定ファイル数**: 約 8 ファイル（実装 6 + テスト 2）
  - **想定行数**: 約 80 行（実装 60 行 + テスト 20 行）

### フェーズ 6: 最終クリーンアップ

- [x] **6-1**: 最終クリーンアップと動作確認
  - 全テストが通ることを確認
...
- **providersテーブルの削除**: Providerモデルは削除済みだが、データベーステーブル自体の削除は別タスクで対応
