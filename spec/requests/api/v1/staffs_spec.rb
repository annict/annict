# frozen_string_literal: true

describe "Api::V1::Staffs" do
  let(:access_token) { create(:oauth_access_token) }
  let!(:staff) { create(:staff) }

  describe "GET /v1/staffs" do
    before do
      get api("/v1/staffs", access_token: access_token.token)
    end

    context "when added no parameters" do
      it "responses 200" do
        expect(response.status).to eq(200)
      end

      it "gets staff info" do
        expected_hash = {
          "id" => staff.id,
          "name" => staff.name,
          "name_en" => staff.name_en,
          "role_text" => staff.role_text,
          "role_other" => staff.role_other,
          "role_other_en" => staff.role_other_en,
          "sort_number" => staff.sort_number,
          "work" => {
            "id" => staff.anime.id,
            "title" => staff.anime.title,
            "title_en" => "",
            "title_kana" => staff.anime.title_kana,
            "media" => staff.anime.media,
            "media_text" => staff.anime.media_text,
            "released_on" => staff.anime.released_at&.strftime("%Y-%m-%d"),
            "released_on_about" => staff.anime.released_at_about,
            "official_site_url" => staff.anime.official_site_url,
            "wikipedia_url" => staff.anime.wikipedia_url,
            "twitter_username" => staff.anime.twitter_username,
            "twitter_hashtag" => staff.anime.twitter_hashtag,
            "syobocal_tid" => "",
            "mal_anime_id" => staff.anime.mal_anime_id&.to_s,
            "images" => {
              "recommended_url" => staff.anime.recommended_image_url,
              "facebook" => {
                "og_image_url" => staff.anime.facebook_og_image_url
              },
              "twitter" => {
                "mini_avatar_url" => staff.anime.twitter_avatar_url(:mini),
                "normal_avatar_url" => staff.anime.twitter_avatar_url(:normal),
                "bigger_avatar_url" => staff.anime.twitter_avatar_url(:bigger),
                "original_avatar_url" => staff.anime.twitter_avatar_url,
                "image_url" => staff.anime.twitter_image_url
              }
            },
            "episodes_count" => staff.anime.episodes_count,
            "watchers_count" => staff.anime.watchers_count,
            "reviews_count" => staff.anime.work_records_with_body_count,
            "no_episodes" => staff.anime.no_episodes?
          },
          "person" => {
            "id" => staff.resource.id,
            "name" => staff.resource.name,
            "name_kana" => staff.resource.name_kana,
            "name_en" => staff.resource.name_en,
            "nickname" => staff.resource.nickname,
            "nickname_en" => staff.resource.nickname_en,
            "gender_text" => staff.resource.gender_text,
            "url" => staff.resource.url,
            "url_en" => staff.resource.url_en,
            "wikipedia_url" => staff.resource.wikipedia_url,
            "wikipedia_url_en" => staff.resource.wikipedia_url_en,
            "twitter_username" => staff.resource.twitter_username,
            "twitter_username_en" => staff.resource.twitter_username_en,
            "birthday" => staff.resource.birthday&.strftime("%Y-%m-%d"),
            "blood_type" => staff.resource.blood_type,
            "height" => staff.resource.height,
            "favorite_people_count" => staff.resource.favorite_users_count,
            "casts_count" => staff.resource.casts.count,
            "staffs_count" => staff.resource.staffs.count
          }
        }
        expect(json["staffs"][0]).to include(expected_hash)
        expect(json["total_count"]).to eq(1)
        expect(json["next_page"]).to eq(nil)
        expect(json["prev_page"]).to eq(nil)
      end
    end
  end
end
