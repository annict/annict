...
describe "GET /@:username/records", type: :request do
  context "when user signs in" do
    ...
    context "when records exist" do
      ...
      let!(:record_1) { create(:record, :on_episode, user: user, recordable: episode_record, body: "楽しかった") }
  ...
  context "when user does not sign in" do
    ...
    context "when records exist" do
      ...
      let!(:record_1) { create(:record, :on_episode, user: user, recordable: episode_record, body: "楽しかった") }
      let!(:work_record) { create(:work_record) }
      let!(:record_2) { create(:record, :on_work, user: user, recordable: work_record, body: "最高") }
  ...
  context "when the month parameter is attached" do
    ...
    let!(:record_1) { create(:record, :on_work, user: user, recordable: record_1_work_record, body: "最高", watched_at: Time.zone.parse("2020-04-01")) }
    let!(:record_2_work_record) { create(:work_record) }
    let!(:record_2) { create(:record, :on_work, user: user, recordable: record_2_work_record, body: "すごく良かった", created_at: Time.zone.parse("2020-05-01")) }
