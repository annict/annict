...
describe User, type: :model do
  describe "#create_or_last_activity_group!" do
    ...
    context "when itemable is EpisodeRecord object and it has body" do
      ...
      let!(:record) { create(:record, :on_episode, user: user, recordable: episode_record, body: "良かった") }

      it "creates activity group" do
        expect(ActivityGroup.count).to eq 0

        user.create_or_last_activity_group!(record)
    ...
    context "when itemable is EpisodeRecord object and it does not have body" do
      let!(:user) { create :user }
      let!(:episode_record) { create(:episode_record) }
      let!(:record) { create(:record, :on_episode, user: user, recordable: episode_record, body: "") }
    ...
    context "when itemable is WorkRecord object and it has body" do
      ...
      let!(:record) { create(:record, :on_work, user: user, recordable: work_record, body: "良かった") }
    ...
    context "when itemable is WorkRecord object and it does not have body" do
      let!(:user) { create :user }
      let!(:work_record) { create(:work_record) }
      let!(:record) { create(:record, :on_work, user: user, recordable: work_record, body: "") }
    ...
    context "when activity group which itemable_type is same and not single is created but it created more than 12 hour ago" do
      let(:user) { create :user }
