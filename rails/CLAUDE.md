...
- **コード実装**: コード記述後は、必ず以下を確認する:
  ...
  - Linterの実行が成功すること（`make lint` or `bin/erb_lint --lint-all` or `yarn prettier --write "**/*.js" && yarn eslint "**/*.js"`）
  - 関連するテストが通ること（`make test-file FILE=spec/path/to/xxx_spec.rb`）
  - 明らかなランタイムエラーがないこと
- **ドキュメント編集**: Markdownファイルを編集した後は、必ず以下を行う:
  - Prettierの実行 (`yarn prettier --write "**/*.md"`)
...
- フォーマットエラー: `yarn prettier --write "**/*.js"`
