...
# コードフォーマット
make fmt                           # Ruby（自動修正）
make -C /workspace fmt             # JS/TS/CSS/JSON/YAML/Markdown/TOML（Oxfmt）
...
# 5. Oxfmt（JavaScript/CSS/JSON/YAML/Markdown/TOML）
...
- **コード実装**: コード記述後は、必ず以下を確認する:
  ...
  - Linterの実行が成功すること（`make lint` or `bin/erb_lint --lint-all` or `make -C /workspace fmt-check && yarn eslint "**/*.js"`）
