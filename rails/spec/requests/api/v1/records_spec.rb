...
RSpec.describe "GET /v1/records", type: :request do
  it "パラメーターを指定しない場合、200ステータスを返すこと" do
    Timecop.freeze(Time.parse("2017-01-29 08:39:04 +09:00")) do
      access_token = create(:oauth_access_token)
      user = create(:user, :with_profile)
      work = create(:work, :with_current_season)
      episode = create(:episode, work:)
      record = create(:record, user:)
      create(:episode_record, record:, work:, episode:, user:)
  ...
  it "パラメーターを指定しない場合、記録情報を取得できること" do
    Timecop.freeze(Time.parse("2017-01-29 08:39:04 +09:00")) do
      access_token = create(:oauth_access_token)
      user = create(:user, :with_profile)
      work = create(:work, :with_current_season)
      episode = create(:episode, work:)
      record = create(:record, user:)
      episode_record = create(:episode_record, record:, work:, episode:, user:)
  ...
  it "アクセストークンが無効な場合、401ステータスを返すこと" do
    get api("/v1/records", access_token: "invalid_token")

    expect(response.status).to eq(401)
  end

  it "記録が存在しない場合、空の配列を返すこと" do
    access_token = create(:oauth_access_token)

    get api("/v1/records", access_token: access_token.token)

    expect(response.status).to eq(200)
    expect(json["records"]).to eq([])
