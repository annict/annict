...
RSpec.describe "DELETE /settings/muted_users/:mute_user_id", type: :request do
  ...
  it "他のユーザーのミュート情報を削除しようとしたとき、エラーが発生すること" do
    user_1 = FactoryBot.create(:registered_user)
    user_2 = FactoryBot.create(:registered_user)
    ...
    expect { delete "/settings/muted_users/#{mute_user.id}" }.to raise_error(ActiveRecord::RecordNotFound)
  end

  it "存在しないミュート情報を削除しようとしたとき、エラーが発生すること" do
    user_1 = FactoryBot.create(:registered_user)

    login_as(user_1, scope: :user)

    expect { delete "/settings/muted_users/999999" }.to raise_error(ActiveRecord::RecordNotFound)
  end
end
