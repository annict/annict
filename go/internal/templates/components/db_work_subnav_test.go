...
// TestDBWorkSubnavは、サブナビが作品の各サブリソースへリンクし、項目にラベルを付け、
// ナビゲーションランドマークを持ち、現在ページ (作品編集) の項目にaria-currentを付ける
// ことを検証する。
func TestDBWorkSubnav(t *testing.T) {
	...
	expectedContents := []string{
		...
		// ラベルはRailsの作品サブナビ (noun.*) に合わせ、番組情報 / 放送予定は日本語の
		// 放送寄りの表現を保つ。
		"エピソード",
		"番組情報",
		"放送予定",
		...
		"スタッフ",
		"作品画像",
		// 作品編集ページが現在ページのため、その項目に印が付き塗りつぶされる。
		`aria-current="page"`,
		`aria-[current=page]:bg-primary`,
		// 項目は折り返さずリスト自身が横スクロールするため、小さい画面でも文書全体が
		// 横に溢れることはない。
		`flex-nowrap`,
		`overflow-x-auto`,
		`whitespace-nowrap`,
...
// TestDBWorkSubnav_MarksCurrentEntryOnSubPageは、項目の配下のページを開いている間も
...
func TestDBWorkSubnav_MarksCurrentEntryOnSubPage(t *testing.T) {
	...
	if !strings.Contains(episodes, `aria-current="page"`) {
		t.Error("エピソードの配下のページでエピソードの項目にaria-currentが付いていません")
	}
...
// TestDBWorkSubnav_MarksNamedSectionOutsideItsPathは、ページ自身のURLが所属する項目の
// 外にある場合 (エピソード基点のエピソード編集フォーム) に、リクエストパスではなく
// CurrentSectionPathでその項目に印が付くことを検証する。
func TestDBWorkSubnav_MarksNamedSectionOutsideItsPath(t *testing.T) {
	...
	episodes := subnavItemHTML(t, html, "/db/works/1/episodes")
	if !strings.Contains(episodes, `aria-current="page"`) {
		t.Error("名指しされたエピソードの項目にaria-currentが付いていません")
	}
	...
	for _, path := range []string{
		...
		if strings.Contains(subnavItemHTML(t, html, path), `aria-current="page"`) {
			t.Errorf("名指しされていない%qの項目にaria-currentが付いています", path)
		}
	}
}

// subnavItemHTMLは指定したパスへリンクするサブナビ項目のマークアップを返す。
...
// TestDBWorkSubnav_OmitsEpisodeItemsWhenNoEpisodesは、作品が「エピソード無し」の
// ときエピソード由来の項目 (エピソード・放送予定) が隠れることを検証する (Railsに
// 合わせている)。
...
// TestDBWorkSubnav_I18nはランドマーク名と項目ラベルが言語ごとに切り替わることを
// 検証する。
