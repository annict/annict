...
// TestWorkPicture_WithImageは画像が登録されている作品がwebp / jpegのソースと
// 呼び出し側のalt・クラスを描画し、枠を埋めるよう切り抜かれるのではなく3:4の枠内に
...
// TestWorkPicture_WithoutImageはフォールバックを検証する。プレースホルダーは
// <source> 無し (サイズ違いの派生が無いため) で実サムネイルと同じ枠に描画され、作品を写した
// 画像ではなく装飾として扱われるようaltは空になる。
...
// TestWorkPicture_WidthDrivesHeightはheight属性が作品画像の比率でwidthに
// 追従し、1つのコンポーネントで異なる表示サイズに対応できることを検証する。
func TestWorkPicture_WidthDrivesHeight(t *testing.T) {
	...
	if !strings.Contains(html, `width="280"`) || !strings.Contains(html, `height="373"`) {
		t.Errorf("width=280に対してheight=373が描画されるべきです: %s", html)
	}
}

// TestWorkPicture_NoClassはClassを省略したときobject-fitのクラスだけが残り、
// class属性の先頭に余分な空白が入らないことを検証する。
