...
// editTestDataは編集のために開いたエピソードのページデータを返す。どの値がどの入力欄へ
...
func TestEdit_StoredValues(t *testing.T) {
	...
	expectedContents := []string{
		// 見出しは作品を名指しし、共有のサブナビはそのフォームへ戻るリンクを持つ。
		"テストアニメ",
		`href="/db/works/1/edit"`,
		// フォームはエピソード自身を更新する。HTMLフォームがPATCHを送るための
		// メソッドオーバーライドを通す。
		`action="/db/episodes/5"`,
		`method="POST"`,
		`name="_method" value="PATCH"`,
		`value="test-csrf-token"`,
		// フォームを開いた時点の版が値と一緒に送信される。古い送信を黙って上書きせずに
		// 却下できるようにするため。
		...
		`id="sort_number"`,
		`value="200"`,
		// 2つの話数の欄は、textの入力欄のままタッチキーボードに数字キーパッドを
		...
		// 編集のために開いたフォームは先頭の欄にフォーカスして始まる。カーソルを受け取る
		// エラー要約が無いため。
		"autofocus",
		"一覧に戻る",
		"更新する",
	...
	for _, hint := range keyboardHints {
		if input := editInputHTML(t, html, hint.field); !strings.Contains(input, hint.want) {
			t.Errorf("%qの入力欄に%sがありません: %s", hint.field, hint.want, input)
		}
	}
}

// TestEdit_MarksEpisodesSubnavEntryは、リクエストパスからは位置を決められないページの
...
func TestEdit_MarksEpisodesSubnavEntry(t *testing.T) {
	...
	// ページはエピソード一覧へ2回リンクする (見出しの操作とサブナビ) ため、戻る
	// リンクではなくサブナビの項目に届くようnavから探し始める。
	html := buf.String()
	...
	if !strings.Contains(item[:end], `aria-current="page"`) {
		t.Error("エピソード編集ページでエピソードの項目にaria-currentが付いていません")
	}
}

// TestEdit_EmptyValuesは編集できる任意カラムが未設定のエピソードを検証する。各欄は、
// 編集者が入力の前に消す必要のあるプレースホルダーではなく空で開く。NULLのupdated_atは
...
func TestEdit_EmptyValues(t *testing.T) {
	...
	for _, field := range []string{"number", "raw_number", "title", "title_en"} {
		if input := editInputHTML(t, html, field); !strings.Contains(input, `value=""`) {
			t.Errorf("%qの欄が空で描画されていません: %s", field, input)
		}
	}
	if !strings.Contains(html, `name="updated_at" value="null"`) {
		t.Error("NULLのupdated_atが明示的な版として描画されていません")
	}
}

// editInputHTMLは指定したidを持つフォーム入力欄のマークアップを返す。
func editInputHTML(t *testing.T, html string, id string) string {
	t.Helper()

	at := strings.Index(html, `id="`+id+`"`)
	if at < 0 {
		t.Fatalf("%qの入力欄が描画されていません", id)
	}
	start := strings.LastIndex(html[:at], "<input")
	if start < 0 {
		t.Fatalf("%qの入力欄の開始タグが見つかりません", id)
	}
	end := strings.Index(html[start:], ">")
	if end < 0 {
		t.Fatalf("%qの入力欄が閉じられていません", id)
	}

	return html[start : start+end+1]
}

// TestEdit_WithErrorsは送信が却下された後のページを検証する。メッセージが冒頭に要約され、
...
func TestEdit_WithErrors(t *testing.T) {
	...
	expectedContents := []string{
		"整数を入力してください",
		// 要約はフィールドを可視ラベルで名指しし、その入力欄へリンクする。
		"ソート番号",
		`href="#sort_number"`,
		`aria-invalid="true"`,
		// 入力欄は自身のエラーメッセージを説明として指す。値が却下された理由を、要約を
		// 読む人だけでなく、欄に降り立った人にも届けるため。
		`aria-describedby="sort_number-error-1"`,
		`id="sort_number-error-1"`,
		// 却下された値はそのまま返し、送信は入力し直しではなく手直しで済むようにする。
		`value="abc"`,
		// 版は編集者が送ったものをそのまま返す。サーバーの現在値に差し替えると、最初の
		// 送信を直している間に書かれた変更を、手直し後の送信が上書きしてしまう。
		`name="updated_at" value="2026-08-12T09:30:15.123456Z"`,
	}
	for _, expected := range expectedContents {
		if !strings.Contains(html, expected) {
			t.Errorf("レスポンスに%qが含まれていません\nHTML: %s", expected, html)
		}
	}

	// エラー要約が読み込み時にフォーカスを受け取るため、先頭の欄はautofocusを手放す。
	// 送信の失敗理由を述べる要約を飛び越えてカーソルが移らないようにするため。
	if strings.Contains(editInputHTML(t, html, "number"), "autofocus") {
		t.Error("エラー要約があるフォームで先頭の欄にautofocusが付いてはいけません")
	}
}

// TestEdit_FallsBackToGenericHeadingは、表示できる名前が無い作品のエピソードを検証する。
...
// TestEdit_DecorativeIconsAreHiddenは編集ページのアイコンのうち、隣接する可視テキストと
// 意味が重複するものを検証する。ボタンとリンクはテキストですでに意味を伝えるため、SVGは
...
func TestEdit_DecorativeIconsAreHidden(t *testing.T) {
	...
	for _, iconName := range []string{"floppy-disk-regular", "arrow-bend-up-left-regular"} {
		...
		if !strings.Contains(html, want) {
			t.Errorf("装飾アイコン%qがaria-hiddenの要素内にありません", iconName)
		}
	}
}

// TestEdit_ConflictShowsStoredValuesは、他者が先にエピソードを書いたために却下された送信を
...
func TestEdit_ConflictShowsStoredValues(t *testing.T) {
	...
	expectedContents := []string{
		...
		"先に保存したタイトル",
		"第3話",
		// 送信された値はフォームに残る。どちらを残すかを決めるのは編集者であり、入力が
	...
	// 案内が並べる保存済みの値には、そのエピソードが未設定にしているカラムも含まれる。
...
// TestEdit_WithoutConflictOmitsStoredValuesは通常の編集ページから案内を外していることを
// 検証する。編集のために開いたフォームは保存済みの値を各欄に既に表示しており、それを繰り返すと
// 対立するもう1組の値のように読めてしまうため。
...
// TestEdit_GuidelineLinkは、エピソード編集ページが見出しの下にエピソードの編集ガイド
...
func TestEdit_GuidelineLink(t *testing.T) {
	...
	for _, expected := range []string{
		// リンクは行き先を名乗り、エピソードの編集ガイドラインを指す。編集途中の内容を
		// フォームに残せるよう、tabnabbing対策付きで新しいタブに開く。
		"エピソードの編集ガイドライン",
