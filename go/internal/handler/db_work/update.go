...
// UpdateはAnnict DB管理画面の作品更新リクエスト (PATCH /db/works/:id) を処理する。
	...
	if err != nil {
		...
		if ae := model.AsAppError(err); ae != nil {
			...
			case model.AppErrCodeConflict:
				// フォームを開いてから本送信までの間に、他者がその作品を書いた。送信された
...
// editFormStateは再描画する編集ページのうち送信に依存する部分。返すステータスと、送信の
// 何が問題だったかを持つ。
type editFormState struct {
	Status     int
	FormErrors *model.ValidationError
	// Conflictは、他者が先にその作品を書いたために送信が却下されたことを表す。ページは
...
// renderRejectedUpdateは適用されなかった送信に対して作品編集フォームを再描画し、送信された
...
func (h *Handler) renderRejectedUpdate(w http.ResponseWriter, r *http.Request, input usecase.UpdateWorkInput, state editFormState) {
	...
	component := layouts.Db(
		...
		db_works.Edit(db_works.EditPageData{
			...
			// バリデーションはworkの読み込みより前に走るため、ここでは保存済みの
...
// editFormRejectionDataは再描画するページが送信された値の他に必要とするものを読み込み、
// 自ら応答を書いた場合にfalseを返す。競合では保存済みのwork全体を読む。送信された値と並べて
// 表示する値と、2回目の送信が名乗るべき版の両方をそこから得るため。formInputはその版に載せ替え、
// 編集者がいま示された行だけを上書きするようにする。それ以外の却下では選択欄の選択肢だけを必要と
// し、formInputは送信された版を運んだままにする。
func (h *Handler) editFormRejectionData(
	...
	if err != nil {
		// 競合を報告する時点でworkが失われている。送信と見比べる対象も、送信し直す先の
		// ページも残っていない。
