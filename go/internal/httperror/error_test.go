...
// TestNotFound_HTMXRedirectは、HTMXリクエストが全画面ページへ送られることを固定する。
// 別タブで先に消された行のボタンは404を受け取り、DB一覧の発行するhx-deleteはhx-targetを
...
func TestNotFound_HTMXRedirect(t *testing.T) {
	...
	if got := rr.Header().Get("HX-Redirect"); got != NotFoundPath {
		t.Errorf("HX-Redirect = %q、期待値 = %q", got, NotFoundPath)
	}
	if got := rr.Header().Get("Location"); got != "" {
		t.Errorf("リダイレクト先 = %q、期待値 = 空", got)
	}
	assertErrorResponse(
		t,
		rr,
...
// TestNotFound_HTMXRequestOnPageIsNotRedirectedは、ページ本体がHX-Redirectを返さない
...
// TestForbidden_HTMXRedirectは、HTMXリクエストが全画面ページへ送られることを固定する。
// DB一覧の削除・非公開ボタンはhx-targetを指定していないhx-deleteのため、そのままでは
// 403の文書が押したボタンの中にスワップされる。
func TestForbidden_HTMXRedirect(t *testing.T) {
	...
	if got := rr.Header().Get("HX-Redirect"); got != ForbiddenPath {
		t.Errorf("HX-Redirect = %q、期待値 = %q", got, ForbiddenPath)
	}
	if got := rr.Header().Get("Location"); got != "" {
		t.Errorf("リダイレクト先 = %q、期待値 = 空", got)
	}
...
// TestForbidden_HTMXRequestOnPageIsNotRedirectedは、ページ本体がHX-Redirectを返さない
// ことを固定する。ここは拒否された要求が送るリダイレクトの遷移先であり、HTMXリクエストに
...
// TestInvalidCSRFToken_HTMXRequestOnPageIsNotRedirectedは、ページ本体がHX-Redirectを
...
func TestInvalidCSRFToken_HTMXRedirect(t *testing.T) {
	...
	if got := rr.Header().Get("HX-Redirect"); got != InvalidCSRFTokenPath {
		t.Errorf("HX-Redirect = %q、期待値 = %q", got, InvalidCSRFTokenPath)
	}
	if got := rr.Header().Get("Location"); got != "" {
		t.Errorf("リダイレクト先 = %q、期待値 = 空", got)
	}
...
// TestInvalidCSRFToken_DoesNotReuseForbiddenMessageはCSRFのページが専用の文言を持つことを
// 固定する。Forbiddenの文言は権限不足の説明で、起きたことと読み手が取るべき行動のどちらとも
...
func TestInvalidCSRFToken_DoesNotReuseForbiddenMessage(t *testing.T) {
	...
	if body := rr.Body.String(); strings.Contains(body, i18n.T(ctx, "error_forbidden_message")) {
		t.Error("CSRFエラーページに権限不足の文言が使われています")
	}
}
...
func TestInternalServerError(t *testing.T) {
	...
	if strings.Contains(rr.Body.String(), "Internal Server Error") {
		t.Error("500レスポンスに内部エラーの文言を含めてはいけません")
	}
}

// TestInternalServerError_HTMXRedirectは、HTMXリクエストが全画面ページへ送られることを
// 固定する。DB一覧の削除・非公開ボタンはhx-targetを指定していないhx-deleteのため、そのまま
// では500の文書が押したボタンの中にスワップされる。
func TestInternalServerError_HTMXRedirect(t *testing.T) {
	...
	if got := rr.Header().Get("HX-Redirect"); got != InternalServerErrorPath {
		t.Errorf("HX-Redirect = %q、期待値 = %q", got, InternalServerErrorPath)
	}
	if got := rr.Header().Get("Location"); got != "" {
		t.Errorf("リダイレクト先 = %q、期待値 = 空", got)
	}
...
// TestInternalServerError_HTMXRequestOnPageIsNotRedirectedは、ページ本体がHX-Redirectを
...
// TestBadGateway_HTMXRequestIsNotRedirectedは、502が専用のページを持たない唯一のエラーで
// あることを固定する。HTMXリクエストの送り先となるルートを持たないため、その場で応答し、他の
// レスポンスと同じようにスワップされる。これは漏れではなく意図した線引きで、502が返るのは
// Railsの配信するパスに対するリバースプロキシの失敗であるのに対し、リクエスト元自身へ
// スワップしてしまう要求はGoの配信するページから発行されるため。
...
// TestNoindexIsDeclaredOnEveryResponseは、どのエラーレスポンスもクローラーに索引しない
// よう伝えることを固定する。/errors/* のルートはGETに応じており、robots.txtが禁じているのは
// /db/ だけのため、これらはサイトの中身を持たないままクロール可能なURLとして存在している。
// 索引から外れること自体は4xx / 5xxのステータスで既に満たされており、本宣言はそれをページ側
// でも述べるもの。
func TestNoindexIsDeclaredOnEveryResponse(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "認可の403", render: Forbidden},
		{name: "CSRFの403", render: InvalidCSRFToken},
...
func assertErrorResponse(
	...
	if rr.Code != wantStatus {
		t.Errorf("ステータスコード = %d、期待値 = %d", rr.Code, wantStatus)
	}
	...
	for _, expected := range []string{
		...
		if !strings.Contains(body, expected) {
			t.Errorf("レスポンスボディに含まれていない文字列 = %q", expected)
		}
	}
}
