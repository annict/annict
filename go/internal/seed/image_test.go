...
func TestGenerateShrineImageData(t *testing.T) {
	...
	if data.Master.ID != expectedID {
		t.Errorf("Master.ID = %q、期待値 = %q", data.Master.ID, expectedID)
	}

	if data.Master.Storage != "store" {
		t.Errorf("Master.Storage = %q、期待値 = %q", data.Master.Storage, "store")
	}

	if data.Master.Metadata.Filename != img.Filename {
		t.Errorf("Master.Metadata.Filename = %q、期待値 = %q", data.Master.Metadata.Filename, img.Filename)
	}

	if data.Master.Metadata.Size != img.Size {
		t.Errorf("Master.Metadata.Size = %d、期待値 = %d", data.Master.Metadata.Size, img.Size)
	}

	if data.Master.Metadata.MimeType != "image/png" {
		t.Errorf("Master.Metadata.MimeType = %q、期待値 = %q", data.Master.Metadata.MimeType, "image/png")
	}

	if data.Master.Metadata.Width != img.Width {
		t.Errorf("Master.Metadata.Width = %d、期待値 = %d", data.Master.Metadata.Width, img.Width)
	}

	if data.Master.Metadata.Height != img.Height {
		t.Errorf("Master.Metadata.Height = %d、期待値 = %d", data.Master.Metadata.Height, img.Height)
	}
}
...
func TestGenerateRandomWorkImage(t *testing.T) {
	...
	if len(img.Filename) < len("master-") || img.Filename[:7] != "master-" {
		t.Errorf("ファイル名の形式 = %q、期待値 = master-{hash}.png", img.Filename)
	}
	...
	if len(img.Path) <= len(expectedPathPrefix) || img.Path[:len(expectedPathPrefix)] != expectedPathPrefix {
		t.Errorf("パスの形式 = %q、期待値 = %qで始まること", img.Path, expectedPathPrefix)
	}

	// サイズが正の値であることを確認
	if img.Size <= 0 {
		t.Errorf("サイズ = %d、期待値 = 1以上", img.Size)
	}

	// 画像の幅と高さを確認 (600x800px)
	if img.Width != WorkImageWidth {
		t.Errorf("画像の幅 = %d、期待値 = %d", img.Width, WorkImageWidth)
	}
	if img.Height != WorkImageHeight {
		t.Errorf("画像の高さ = %d、期待値 = %d", img.Height, WorkImageHeight)
	}
...
func TestGenerateRandomProfileImage(t *testing.T) {
	...
	if img.Width != ProfileImageWidth {
		t.Errorf("画像の幅 = %d、期待値 = %d", img.Width, ProfileImageWidth)
	}
	if img.Height != ProfileImageHeight {
		t.Errorf("画像の高さ = %d、期待値 = %d", img.Height, ProfileImageHeight)
	}
