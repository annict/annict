...
// CreateProfileImageParamsプロフィール画像作成のパラメータ
type CreateProfileImageParams struct {
	ProfileID int64
	UserID    model.UserID
}

// CreateProfileImageResultプロフィール画像作成の結果
type CreateProfileImageResult struct {
	ProfileID int64
	ImagePath string
}

// CreateProfileImageUsecaseプロフィール画像生成Usecase (シード専用、バルクインサート対応)
...
// NewCreateProfileImageUsecase新しいCreateProfileImageUsecaseを作成
...
// ExecuteBatch複数のプロフィール画像をバッチで作成します
// 各プロフィール画像について：
// 1. ランダム画像を生成
...
// ExecuteBatchWithTx複数のプロフィール画像をバッチで作成します (テスト用：既存トランザクションを使用)
...
// createSingleProfileImage単一のプロフィール画像を作成します
